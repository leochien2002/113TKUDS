class InsertionSortDemo {
    ...
    /**
     * 二分插入排序：使用二分搜尋找到插入位置
     * 時間複雜度：最佳 O(n log n)，最壞 O(n²)，平均 O(n²)
     * 空間複雜度：O(1)
     * 說明：
     * 最佳情況：$O(n \log n)$。當陣列已排序時，二分搜尋每次操作都是 $O(\log i)$，總比較次數為 $O(n \log n)$。但元素移動仍為 $O(1)$，總移動次數仍為 $O(n)$。
     * 最壞情況：$O(n^2)$。當陣列反向排序時，每次都需要移動約 $i$ 個元素，總移動次數為 $O(n^2)$。二分搜尋的比較次數為 $O(n \log n)$，但移動操作佔主導。
     * 平均情況：$O(n^2)$。儘管二分搜尋降低了比較次數，但元素移動的次數在平均情況下仍然是 $O(n^2)$，因為需要為新元素騰出空間。
     */